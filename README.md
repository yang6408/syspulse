...
| 유스케이스 명 | 실시간 메트릭 조회 |
| 액터 | 사용자 |
| 사전 조건 | VM이 등록되어 있고 Node Exporter가 실행 중 |
| 기본 흐름 | 1. 사용자가 VM 목록에서 VM 선택 → 2. React가 5초마다 `/api/metrics/current` 호출 → 3. 백엔드가 Prometheus에 PromQL 쿼리 → 4. 결과를 가공하여 JSON 응답 → 5. 화면에 현재값 카드 업데이트 |
| 예외 흐름 | VM이 오프라인인 경우 메트릭 값 0으로 표시 |
| 사후 조건 | CPU/메모리/디스크/네트워크 현재값이 화면에 표시됨 |
...
| 유스케이스 명 | 메트릭 이력 조회 |
| 액터 | 사용자 |
| 사전 조건 | metric_history 테이블에 데이터가 존재 |
| 기본 흐름 | 1. 사용자가 시간 범위 선택 → 2. React가 `/api/metrics/history?vm_id=&range=` 호출 → 3. 백엔드가 PostgreSQL에서 해당 범위 데이터 조회 → 4. JSON 응답 → 5. 차트에 이력 데이터 표시 |
| 예외 흐름 | 해당 시간대 데이터 없으면 빈 차트 표시 |
| 사후 조건 | 선택한 시간 범위의 메트릭 이력이 차트에 표시됨 |
...
| 액터 | 사용자 |
| 사전 조건 | VM이 선택되어 있음 |
| 기본 흐름 | 1. 사용자가 "직접 입력" 버튼 클릭 → 2. 날짜/시간 선택 드롭다운 표시 → 3. 시작/끝 날짜·시간 선택 → 4. "조회" 버튼 클릭 → 5. 해당 범위 데이터 조회 및 차트 갱신 |
| 예외 흐름 | 시작 시간이 끝 시간보다 늦으면 경고 알림 표시 |
| 사후 조건 | 지정한 시간 범위의 데이터가 차트에 표시됨 |
...
export default function useMetrics(vmIp, vmId, range = '1h', customRange = null) {
  ...
  useEffect(() => {
    if (!vmId) return
    const fetch = () => {
      const url = customRange?.start
        ? `/api/metrics/history?vm_id=${vmId}&start=${customRange.start}&end=${customRange.end}`
        : `/api/metrics/history?vm_id=${vmId}&range=${range}`
  ...
  for (const vm of vms) {
    ...
    const cpu = await queryPrometheus(
      `100 - (avg by(instance)(rate(node_cpu_seconds_total
       {mode="idle",instance="${instance}"}[1m])) * 100)`
    );
    // memory, disk, network 동일 방식으로 수집
    await db.query(
...
| 항목 | 내용 |
|------|------|
| 알림 기능 | 임계치 초과 시 팝업 알림 및 이력 저장 미구현 |
| CSV 내보내기 | 메트릭 데이터 CSV 다운로드 미구현 |
| 사용자 인증 | 로그인 기능 없어 내부 네트워크 전용으로만 사용 가능 |
| HTTPS | SSL 인증서 미적용으로 보안 취약 |
| 데이터 보존 | 오래된 메트릭 데이터 자동 삭제 정책 미적용 |

향후에는 알림 기능 구현, CSV 내보내기, 사용자 인증, HTTPS 적용 등을 추가하여 실제 운영 환경에서도 활용 가능한 수준의 모니터링 시스템으로 발전시킬 계획이다.

---

**GitHub 저장소:** https://github.com/yang6408/syspulse
