...
| 이름 | 실시간 메트릭 조회 |
| 액터 | 사용자 |
| 사전조건 | VM이 등록되어 있고 Node Exporter가 실행 중 |
...
| 사후조건 | CPU/메모리/디스크/네트워크 현재값이 화면에 표시됨 |
...
| 이름 | 메트릭 이력 조회 |
| 액터 | 사용자 |
| 사전조건 | metric_history 테이블에 데이터가 존재 |
...
| 사후조건 | 선택한 시간 범위의 메트릭 이력이 차트에 표시됨 |
...
| 액터 | 사용자 |
| 사전조건 | VM이 선택되어 있음 |
| 기본흐름 | 1. 사용자가 "직접 입력" 버튼 클릭 → 2. 날짜/시간 선택 드롭다운 표시 → 3. 시작/끝 날짜·시간 선택 → 4. "조회" 버튼 클릭 → 5. 해당 범위 데이터 조회 및 차트 갱신 |
| 예외흐름 | 시작 시간이 끝 시간보다 늦으면 경고 알림 표시 |
| 사후조건 | 지정한 시간 범위의 데이터가 차트에 표시됨 |
...
향후에는 사용자 인증, HTTPS 적용 등을 추가하여 실제 운영 환경에서도 활용 가능한 수준의 모니터링 시스템으로 발전시킬 계획이다.
